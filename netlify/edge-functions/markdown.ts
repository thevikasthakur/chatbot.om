 ...
 *
 * `scripts/ai-readability.mjs` writes a Markdown twin into `public/` for every
 * page, but those files are an implementation detail: they are the payload this
 * function serves on an internal rewrite, and any request that asks for one
 * directly is redirected to the canonical page. That keeps one address per
 * piece of content, so there is nothing to canonicalise away and nothing for an
 * agent to cite by the wrong name.
 *
...
export default async function handler(request: Request, context: Context) {
  // Never touch form posts or anything else that changes state.
...
export const config: Config = {
  path: "/*",
  excludedPath: ["/_next/*", "/api/*", "/hero/*", "/teasers/*"],
};
