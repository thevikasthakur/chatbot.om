/**
 * Shared validation for the Chatbot.OM blog content store.
 *
...
// The sister product (the full customer care suite). Links to it are
// editorial cross-recommendations, not evidence: allowed without a sources[]
// entry, capped at one per article, and never listed as a source.
const SISTER_DOMAIN_RE = /^https:\/\/(www\.)?customercare\.om/i;
...
const REGULATED_CATEGORIES = new Set(["Rules & Updates"]);
...
function validatePolicy(article, config, errors) {
  ...
  const brand = settings.brandName ?? "Chatbot.OM";
  const mentionRe = /chatbot\.om/gi;
  let m;
...
function validateLinksAndSources(article, config, allArticles, errors) {
  ...
  const regulated = new Set(["Rules & Updates"]);
