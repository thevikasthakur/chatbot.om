/**
 * Daily article generator for the Chatbot.OM blog.
 *
...
const SITE_ORIGIN = "https://chatbot.om";
const AUTHOR = "Chatbot.OM Editorial Team";
...
const SISTER = {
  ...
  origin: "https://customercare.om",
  articlesDir: "/Users/thakur/Workspace/voxcare-static/content/articles",
  pages: [
    { path: "/", title: "CustomerCare.OM home", context: "full customer care suite, voice agents, phone line answered in Omani Arabic, enterprise" },
    { path: "/comparison/", title: "CustomerCare.OM comparison", context: "alternatives and product fit, which product suits which business" },
    { path: "/product/omani-arabic-ai-customer-care/", title: "Omani Arabic AI Customer Care", context: "inbound phone calls answered by a voice agent in Omani Arabic, IVR replacement" },
    { path: "/product/outbound-calling-ai-agents/", title: "Outbound Calling AI Agents", context: "outbound calling campaigns, reminders and follow-ups by voice" },
    { path: "/product/ai-email-agent/", title: "AI Email Agent", context: "a dedicated agent working the whole email inbox, triage and drafting" },
    { path: "/enterprise/", title: "Enterprise", context: "large deployments, IVR and agent floors, procurement, SIP trunk" },
  ],
};
...
1. Search the last 7 to 10 days of announcements from: MTCIT, TRA, the Ministry of Commerce Industry and Investment Promotion, Oman News Agency, reputable Omani outlets (Oman Observer, Times of Oman, Muscat Daily), and WhatsApp Business Platform / Meta business product news.
2. If you find ONE development that materially affects how small Omani businesses sell, message and serve customers online (a messaging or consent rule, a platform policy change, a scam wave targeting business accounts, an e-commerce rule, a fee change, a festival programme that moves demand online), write about that instead of the fallback topic. Explain what changed, who it applies to, from when, and exactly what a small business should do this week.
...
5. The sister site covers the same week for operations leads at large service organisations; your reader is the small message-first business. If the week's story only matters to enterprises, leave it to them and take the fallback.
`
    : "";
...
`
  ...
  return `Write one article for the Chatbot.OM blog (chatbot.om/blog), the plain-language library of Chatbot.OM, Oman's own sovereign chatbot: website chat and WhatsApp answered around the clock for Omani businesses.

THE READER
Two people share this article over WhatsApp: the person who runs a small Omani business from their phone, and the customer messaging businesses like theirs. The first reader owns or manages a team of roughly two to ten: a boutique, a restaurant, a gym, a salon, a guesthouse, a tuition centre, a rental office, a clinic, a small online store. Their enquiries arrive as messages: WhatsApp first, then the website and Instagram. They answer between tasks, often at night, and nobody on the team is technical. The test for every paragraph: would a boutique owner in Al Mouj or the manager of a Seeb restaurant understand it on first read, and would they forward it to a friend who runs something because it genuinely helps? The reader should know within the first three sentences how the topic touches their money, time, rights or peace of mind.

THE TARGET BUSINESS (who the worked examples are about)
- Every worked example, composite persona and cost comparison must be sized for a small, message-first business: two to ten people, enquiries arriving on WhatsApp and the website, the owner close to every conversation. Restaurants and cafes, salons and spas, gyms, boutiques and online sellers, guesthouses, tuition centres, rental and property offices, small clinics, workshops.
- Never build the article's case around an enterprise with an IVR, a switchboard or a floor of agents; that is the sister site's reader. Large operations may appear at the edge of a story, and as the upgrade path when a business outgrows chat.
- The strongest angles for this audience: the message answered in five minutes versus never, the owner drowning in chats, the enquiry that becomes a booking without a phone call, WhatsApp done properly (and legally), and the website that finally earns its keep.
...
- The brand is written exactly "Chatbot.OM", never "Chatbot OM", "ChatbotOM" or "chatbot.om" as a name in prose (the lowercase form is only the domain in links). The sister product is written exactly "CustomerCare.OM".
...
4. One "quote" block giving a voice to the industry the article touches: your clearly composite persona (the boutique owner, the restaurant manager, the gym coordinator, the home business founder) saying one line they would genuinely say, under 24 words. Set "persona": true and attribute with name, role and place, for example "Rahma, boutique owner, Al Mouj". For a real public quotation instead, "sourceUrl" is required and the quote must be verifiable word for word. Never invent a quote from a real person.
...
- CustomerCare.OM (https://customercare.om) is this site's bigger sister: the full customer care suite. Everything Chatbot.OM does, plus inbound phone calls answered by a voice agent in Omani Arabic, outbound calling campaigns, and a dedicated agent working a whole email inbox. Built for organisations running an IVR and a floor of human agents; typically live in about a week. Both products keep data inside Oman on the same platform family, so knowledge, history, tickets and leads carry across when a business upgrades.
- When a problem in the article is genuinely phone-shaped (the phone line is the main way customers reach the business, calls are still the bulk of the work even with good chat, outbound calling is needed, or the reader is an organisation with agents and an IVR), the honest recommendation is CustomerCare.OM. Say so plainly in one natural sentence with one link. Chat first is still the family's advice: a good chat presence often reduces call volume, and when calls remain the bulk of the work, the suite is the right answer.
- Candidate CustomerCare.OM links for this topic:
${sisterLinkLines}
...
- Chatbot.OM may appear at most once or twice, as a practical aside (for example inside one callout with tone "brand"), never as the article's hero. The article must be fully useful to someone who never clicks anything.
- When a scene involves a customer messaging a business, let them write and speak the way Omani customers actually do, dialect, mixed Arabic and English, and voice notes included, and let understanding that speech be the quiet point. Never write "our USP", never write marketing slogans.
- Natural Chatbot.OM angles, when one fits: it answers the website and WhatsApp around the clock, it understands voice notes in Omani Arabic, it turns chats into tickets, follow-ups and saved leads, it is live in about a day and priced for a small team, and customer data stays inside Oman. Pick at most one. Never claim it answers phone calls; the phone belongs to the sister suite.
- If a call to action is natural, the phrase this site uses is "Get Chatbot.OM"; never "book a demo".
- Never claim certifications, customer counts or awards for Chatbot.OM.
...
PUBLISHED ON THE SISTER SITE, customercare.om (never duplicate these)
${sisterCoverageBlock}
- The sister site writes for operations leads at organisations with staffed phone lines; this site writes for small message-first teams. If your topic brushes against one of theirs, your angle, examples and worked numbers must clearly serve THIS site's reader, or you should choose a different emphasis entirely.
...
function claudeCliStructured(prompt, label, { maxTurns = 40 } = {}) {
  ...
  if (process.env.CHATBOT_OM_CLAUDE_MODEL) {
    args.push("--model", process.env.CHATBOT_OM_CLAUDE_MODEL);
  }
 ...
 */
const OWN_DOMAIN_RE = /^https?:\/\/(www\.)?chatbot\.om/i;
...
async function main() {
  ...
  const dateStr = args.date ?? process.env.CHATBOT_OM_GENERATION_DATE ?? todayInMuscat();
