...
TODAY=$(date +%Y-%m-%d)
SUCCESS_MARKER="/tmp/chatbot-om-article-gen-success-${TODAY}"
...
find /tmp -maxdepth 1 -name "chatbot-om-article-gen-success-*" -mtime +7 -delete 2>/dev/null || true
...
if [ -n "$LAST_COMMIT_DATE" ]; then
  ...
  if [ "$STALE_DAYS" -ge 3 ]; then
    ...
    osascript -e "display notification \"No article published for $STALE_DAYS days. Check scripts/cron.log.\" with title \"Chatbot.OM blog pipeline\"" 2>/dev/null || true
  fi
fi
